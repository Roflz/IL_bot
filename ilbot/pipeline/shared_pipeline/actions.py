    ...
    for gamestate_idx, gamestate_actions in enumerate(raw_action_data):
        ...
        for key_press in gamestate_actions.get('key_presses', []):
            ...
            all_actions.append({
                ...
                'type': 2,  # 2 = key_press
                'x': key_press.get('x', 0),     # Use actual coordinates
                'y': key_press.get('y', 0),
        ...
        for key_release in gamestate_actions.get('key_releases', []):
            ...
            all_actions.append({
                ...
                'x': key_release.get('x', 0),     # Use actual coordinates
                'y': key_release.get('y', 0),
                'button': 0,  # No button for keys
                'key': key_value,
                'scroll_dx': 0,  # No scroll for keys
                'scroll_dy': 0
        ...
        for scroll in gamestate_actions.get('scrolls', []):
            ...
            all_actions.append({
                ...
                'type': 4,  # 4 = scroll
                'x': scroll.get('x', 0),     # Use actual coordinates
                'y': scroll.get('y', 0),
...
def convert_live_features_to_sequence_format(feature_window: np.ndarray) -> np.ndarray:
    """
    Convert live feature window from ilbot.ui.botgui to the format expected by create_temporal_sequences.
...
def convert_live_actions_to_raw_format(action_tensors: List[List[float]], 
    ...
    """
    Convert live action tensors from ilbot.ui.botgui to the raw format expected by convert_raw_actions_to_tensors.
