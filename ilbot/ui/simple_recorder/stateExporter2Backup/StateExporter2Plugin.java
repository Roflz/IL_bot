                ...
                for (int slotId = 0; slotId < items.length; slotId++)
                {
                    ...
                    if (item != null && item.getId() != -1)
                    {
                        ...
                        try
                        {
                            ...
                            boolean isNoted = itemDef.getNote() != -1;
                            slotInfo.put("isNoted", isNoted);
                            
                            // If noted, get the unnoted item name
                            if (isNoted) {
                                try {
                                    ItemComposition unnotedDef = client.getItemDefinition(itemDef.getNote());
                                    slotInfo.put("unnotedItemName", unnotedDef.getName());
                                } catch (Exception e) {
                                    slotInfo.put("unnotedItemName", null);
                                }
                            } else {
                                slotInfo.put("unnotedItemName", itemDef.getName());
                            }
                        ...
                        catch (Exception e)
                        {
                            ...
                            slotInfo.put("quantity", item.getQuantity());
                            slotInfo.put("isNoted", false);
                            slotInfo.put("unnotedItemName", "Unknown");
            ...
            try {
                ...
                if (tutorialOpen) {
                    ...
                    if (nameTextWidget != null) {
                        Map<String, Object> nameTextInfo = new HashMap<>();
                    ...
                    if (setNameWidget != null) {
                        Map<String, Object> setNameInfo = new HashMap<>();
