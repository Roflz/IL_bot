...
class MainWindow:
    ...
    def _toggle_live_view(self):
        ...
        try:
            ...
            # Get the widget names for comparison
            live_frame_name = str(self.live_frame)
            notebook_frame_name = str(self.notebook_frame)
            print(f"🔍 DEBUG [main_window.py:593] live_frame_name: {live_frame_name}")
            print(f"🔍 DEBUG [main_window.py:594] notebook_frame_name: {notebook_frame_name}")
            print(f"🔍 DEBUG [main_window.py:595] live_frame_name in panes: {live_frame_name in panes}")
            print(f"🔍 DEBUG [main_window.py:596] notebook_frame_name in panes: {notebook_frame_name in panes}")
            
            if live_frame_name not in panes:
                error_msg = f"live_frame not found in panes: {panes}"
                print(f"❌ ERROR [main_window.py:597] {error_msg}")
                raise RuntimeError(error_msg)
                
            if notebook_frame_name not in panes:
                error_msg = f"notebook_frame not found in panes: {panes}"
                print(f"❌ ERROR [main_window.py:602] {error_msg}")
                raise RuntimeError(error_msg)
            
            print("🔍 DEBUG [main_window.py:607] Pane validation passed")
            
            if self.live_view_visible:
                print("🔍 DEBUG [main_window.py:612] Hiding live view...")
                
                # Hide the live view by setting its weight to 0
                print("🔍 DEBUG [main_window.py:617] About to set live_frame weight to 0...")
                self.paned.paneconfig(self.live_frame, weight=0)
                print("🔍 DEBUG [main_window.py:622] About to hide live_frame with grid_remove...")
                self.live_frame.grid_remove()  # Hide the frame
                print("🔍 DEBUG [main_window.py:627] About to set live_view_visible to False...")
                self.live_view_visible = False
                
                # Make the notebook frame fill the entire paned window
                print("🔍 DEBUG [main_window.py:632] About to set notebook_frame weight to 1...")
                self.paned.paneconfig(self.notebook_frame, weight=1)
                
                print("🔍 DEBUG [main_window.py:637] About to update status...")
                self.update_status("Live view hidden - tabs expanded", "info")
                print("🔽 Live view hidden - tabs now fill the entire window")
                print("🔍 DEBUG [main_window.py:642] Live view hide completed successfully")
                
            else:
                print("🔍 DEBUG [main_window.py:647] Showing live view...")
                
                # Show the live view by restoring its weight and making it visible
                print("🔍 DEBUG [main_window.py:652] About to show live_frame with grid()...")
                self.live_frame.grid()  # Show the frame
                print("🔍 DEBUG [main_window.py:657] About to set live_frame weight to 7...")
                self.paned.paneconfig(self.live_frame, weight=7)
                print("🔍 DEBUG [main_window.py:662] About to set notebook_frame weight to 3...")
                self.paned.paneconfig(self.notebook_frame, weight=3)
                
                # Restore minimum sizes
                print("🔍 DEBUG [main_window.py:667] About to restore minimum sizes...")
                try:
                    self.paned.paneconfig(self.live_frame, minsize=220)
                    self.paned.paneconfig(self.notebook_frame, minsize=180)
                    print("🔍 DEBUG [main_window.py:672] Minimum sizes restored successfully")
                except Exception as e:
                    print(f"⚠️ WARNING [main_window.py:677] Failed to restore minimum sizes: {e}")
                
                # Restore the sash position
                print("🔍 DEBUG [main_window.py:682] About to schedule sash position restoration...")
                self.root.after_idle(self._restore_sash_position)
                print("🔍 DEBUG [main_window.py:687] Sash position restoration scheduled")
                
                print("🔍 DEBUG [main_window.py:692] About to set live_view_visible to True...")
                self.live_view_visible = True
                print("🔍 DEBUG [main_window.py:697] About to update status...")
                self.update_status("Live view restored", "info")
                print("🔼 Live view restored - normal layout")
                print("🔍 DEBUG [main_window.py:702] Live view show completed successfully")
