#!/usr/bin/env python3
"""
Woodcutting Plan V2 (P2P) - Using Camera V2 System
==================================================

Simple woodcutting plan that cuts normal trees at the Grand Exchange and banks them.
Uses the new autonomous camera system (camera_v2) and click_with_camera_v2.
"""
...
from services.click_with_camera_v2 import click_object_with_camera
from actions.objects import _find_closest_object_in_area, _find_closest_object_by_distance
...
Required Items: Axe"""
    ...
    def set_phase(self, phase: str, camera_setup: bool = True):
        ...
        self.next = phase
        logging.info(f"[{self.id}] Phase set to: {phase}")
    ...
    def _handle_woodcutting(self) -> int:
        ...
        if tree_obj:
            ...
            # Use new camera system - just pass world coordinates
    ...
    def _handle_rising_roots_event(self) -> int:
        ...
        if anima_root_obj:
            ...
            if isinstance(root_x, int) and isinstance(root_y, int):
                result = click_object_with_camera(
